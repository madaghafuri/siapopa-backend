...
import { golonganPestisida, InsertGolonganPestisida } from '../../db/schema/golongan-pestisida.js';
...
stock.post(
  ...
  validator('form', (value, c) => {
    const { tanaman_id, lokasi_id, satuan, opt_id, bahan_aktif_id, merk_dagang, volume, periode_bulan, tahun_pengadaan, golongan_pestisida_id, expired_date } = value as unknown as InsertPestisida;

    if (!tanaman_id || !lokasi_id || !satuan || !opt_id || !bahan_aktif_id || !merk_dagang
      || !merk_dagang || !volume || !periode_bulan || !tahun_pengadaan || !golongan_pestisida_id || !expired_date
    ) {
    ...
    console.log('Validation successful:', { tanaman_id, lokasi_id, satuan, opt_id, bahan_aktif_id, merk_dagang, volume, periode_bulan, tahun_pengadaan, golongan_pestisida_id });
    return { tanaman_id, lokasi_id, satuan, opt_id, bahan_aktif_id, merk_dagang, volume, periode_bulan, tahun_pengadaan, golongan_pestisida_id };
  }),
  async (c) => {
...
stock.get('/stock-pestisida/reload', async (c) => {
  const selectPestisida = await db.select().from(pestisida).orderBy(pestisida.id);
  return c.html(
    <Fragment>
...
stock.get('/stock-pestisida/edit/:id', async (c) => {
  const id = c.req.param('id');
  const pestisidaItem = await db.select().from(pestisida).where(eq(pestisida.id, parseInt(id)));
  const selectGolonganPestisida = await db.select().from(golonganPestisida);
  const selectBahanAktif = await db.select().from(bahanAktif);
  const selectOPT = await db.select().from(opt);
  const selectTanaman = await db.select().from(tanaman);
...
stock.post('/golongan-pestisida/edit/:id', authorizeStockInput, validator('form', (value, c) => {
  ...
  try {
    await db.update(golonganPestisida).set(golonganPestisidaData).where(eq(golonganPestisida.id, parseInt(id)));
  } catch (error) {
...
stock.get('/golongan-pestisida/reload', async (c) => {
  const selectGolonganPestisida = await db.select().from(golonganPestisida).orderBy(golonganPestisida.id);
  return c.html(
    <Fragment>
...
stock.delete('/golongan-pestisida/delete/:id', async (c) => {
  ...
  try {
    await db.delete(golonganPestisida).where(eq(golonganPestisida.id, parseInt(id)));
  } catch (error) {
    return c.html(<span>Terjadi kesalahan dalam proses penghapusan data. Silahkan coba lagi</span>, 500);
  }
...
stock.get('/golongan-pestisida/edit/:id', async (c) => {
  const id = c.req.param('id');
  const golonganPestisidaItem = await db.select().from(golonganPestisida).where(eq(golonganPestisida.id, parseInt(id)));

  return c.html(<ModalGolonganPestisida golonganPestisida={golonganPestisidaItem[0]} />);
});
...
stock.get('/bahan-aktif/reload', async (c) => {
  const selectBahanAktif = await db.select().from(bahanAktif).orderBy(bahanAktif.id);
  return c.html(
    <Fragment>
...
stock.get('/bahan-aktif/edit/:id', async (c) => {
  const id = c.req.param('id');
  const bahanAktifItem = await db.select().from(bahanAktif).where(eq(bahanAktif.id, parseInt(id)));

  return c.html(<ModalBahanAktif bahanAktif={bahanAktifItem[0]} />);
});

stock.post('/bahan-aktif/edit/:id', authorizeStockInput, validator('form', (value, c) => {
  const { nama_bahan } = value as unknown as InsertBahanAktif;
  ...
  try {
    await db.update(bahanAktif).set(bahanAktifData).where(eq(bahanAktif.id, parseInt(id)));
  } catch (error) {
    return c.html(<span>Terjadi kesalahan dalam proses pengeditan data. Silahkan coba lagi</span>, 500);
  }
